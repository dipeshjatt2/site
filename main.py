    ...
    async def scrape_single_quiz(self, quiz_info):
        ...
        try:
            async with aiohttp.ClientSession() as session:
                ...
                with open(output_filename, 'w', encoding='utf-8') as f:
                    for result in sorted(question_results, key=lambda x: x.get('q_num', float('inf')) if isinstance(x, dict) else float('inf')):
                        if isinstance(result, Exception) or "error" in result:
                            q_num = result.get('q_num', -1) if isinstance(result, dict) else -1
                            error_msg = result.get('error', str(result)) if isinstance(result, dict) else str(result)
                            f.write(f"{q_num + 1}. FAILED TO FETCH QUESTION: {error_msg}\n\n")
                            continue
...
def get_task_status(task_id):
    ...
    response = {
        "task_id": task_id, "status": task['status'], "progress": task['processed'],
        "total": task['total'], "progress_percentage": progress,
...
def home():
    return jsonify({"message": "Scraper API is running."})
